...
from domain.model.datesum import DateSum
...
def printHprs(hprs: list[DateSum]):
    for item in hprs:
        print(f"{item.date.strftime(displayDateLayout)} {hprDisplay(item.sum):>8,.1f}%")
...
def printDrawdownInfo(data: DrawdownInfo):
    ...
    print(f"Дата максимума эквити {data.highEquityDate.strftime(displayDateLayout)}")
