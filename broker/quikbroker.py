...
from broker.model import Portfolio, Security, Order, PortfolioLimits


class QuikBroker:

    def __init__(self, port: int, marketDataCallback):
        self._port = port
        self._marketDataCallback = marketDataCallback
    ...
    def onNewCandle(self, data):
        ...
        if candle.dateTime >= self._startCandles:
            self._marketDataCallback(candle)
    ...
    def init(self):
        ...
        self._quik = QuikPy(requests_port=self._port, callbacks_port=self._port + 1)
    ...
    def getLastCandles(self, security: Security, candleInterval: str):
        ...
        new_bars = self._quik.GetCandlesFromDataSource(
            security.classCode, security.code, _quikTimeframe(candleInterval), MaxBars
        )["data"]
    ...
    def getPortfolioLimits(self, portfolio: Portfolio):
        ...
        return PortfolioLimits(
            start_limit_open_pos, used_lim_open_pos, varmargin, fut_accured_int
        )
...
def _quikTimeframe(candleInterval: str):
    if candleInterval == CandleInterval.Minutes5:
        return 5
